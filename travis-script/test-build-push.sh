...
setTag_push_rm(){
    ...
    _do docker push "${ACR_USERNAME }"/"${DOCKER_IMAGE_NAME}":"${TAG}"
    echo "PASSED - Pushed  ${ACR_USERNAME }/${DOCKER_IMAGE_NAME}:${TAG} Successfully!."
    echo "PASSED - Pushed  ${ACR_USERNAME }/${DOCKER_IMAGE_NAME}:${TAG} Successfully!." >> result.log
    echo "INFORMATION: Before rmi - docker images"
    _do docker images
    echo "INFORMATION - RM ""${ACR_USERNAME }"/"${DOCKER_IMAGE_NAME}":"${TAG}"
